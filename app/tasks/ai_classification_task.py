...
class CeleryTaskClientAI(IAsyncTaskClient):
    def run_task(self, task_data):
        task: AsyncResult = ai_classification_task.delay(task_data)
        return task.id


@celery.task(bind=True)
def ai_classification_task(self, kwargs):
    ...
    job_id = _initiate_remote_job(partnumber, progress_channel)
    if not job_id:
...
def _initiate_remote_job(partnumber: str, progress_channel: str):
    try:
        response = requests.post(
            f"{settings.NEXA_AI_SERVER}/process",
            json={"partnumber": partnumber, "progress_channel": progress_channel}
        )
